...
class VGGUnet(nn.Module):
    ...
    def __init__(self, encoder, *, pretrained=False, out_channels=2):
        ...
        if self.pretrained : self.pp = transforms.Normalize(mean=[0.485, 0.456, 0.406],
    ...
    def forward(self, x):
        if self.pretrained: x = self.pp(x)
...
def vgg13bn_unet(output_dim, pretrained: bool=True):
    return VGGUnet(vgg13_bn, pretrained=pretrained, out_channels=output_dim)

def vgg16bn_unet(output_dim, pretrained: bool=True):
    return VGGUnet(vgg16_bn, pretrained=pretrained, out_channels=output_dim)

class VggUnet13Trainer(SegmenTrainer):
    
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.model = vgg13bn_unet(*args, **kwargs)
        self.n_classes = self.model.n_classes 
        
class VggUnet16Trainer(SegmenTrainer):
    
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.model = vgg16bn_unet(*args, **kwargs)
        self.n_classes = self.model.n_classes 